...
const BlogPostForm = ({ onPostCreated }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (password !== 'your-secret-password') { // Replace with secure auth in production
      setError('Incorrect password');
      return;
    }
    try {
      const response = await axios.post('http://localhost:5000/api/blog/posts', { title, content }); // Update URL
